...
{
	int blockFeatureSize = NumOfBins * m_block.width * m_block.height;
...
{
	int blockFeatureSize = NumOfBins * block.width * block.height;
...
IplImage** HOGProcessor::calculateIntegralHOG(IplImage* in)
{
	...
	for (y = 0; y < in->height; y++)
	{
		...
		for (x = 0; x < in->width; x++)
		{
			...
			{					// ta thay mau bang mot dai luong rat nho
				tempGradient = ((atan(yDevValue / 0.00001))*(180 / PI)) + 90;
			}
			else
			{
			...
			if (tempGradient <= 180)
			{
				int bin = (tempGradient <= 20) ? 0 : ceil(tempGradient / 20) - 1;
				ptrs[bin][x] = tempMagnitude;
			}
		}
	}

	cvReleaseImage(&xsobel);	// Destroy
	cvReleaseImage(&ysobel);	// Destroy

								// Tinh integral image tuong ung voi moi bin images
	for (int i = 0; i < NumOfBins; i++)
	{
		cvIntegral(bins[i], integrals[i]);
		cvReleaseImage(&bins[i]);	// Destroy
	}
	cvReleaseImage(bins);	// Destroy
	free(ptrs);
...
CvMat* HOGProcessor::calcHOGWindow(IplImage** integrals, CvRect window, int normalization)
{
	...
	for (blockStartY = window.y; blockStartY <= (window.y + window.height - heightBlockInPixels);
		blockStartY += heightStep)	// overllap
	{
		for (blockStartX = window.x; blockStartX <= (window.x + window.width - widthBlockInPixels);
			blockStartX += widthStep)	// overllap
		{
			cvGetCols(windowFeatureVector, &vectorBlock, startcol, startcol + blockFeatureSize);

			calcHOGForBlock(cvRect(blockStartX, blockStartY, widthBlockInPixels, heightBlockInPixels), &vectorBlock,
				integrals, m_cell, normalization);
...
CvMat* HOGProcessor::calcHOGWindow(IplImage *img, IplImage** integrals, CvRect window, int normalization)
{
	...
	// Kich thuoc vector dac trung cho moi cua so = blockFeatureSize * NumOfBlocks
	int blockFeatureSize = NumOfBins * m_block.height * m_block.width;
	...
	for (blockStartY = window.y; blockStartY <= (window.y + window.height - cellHeight * blockHeight);
		blockStartY += cellHeight * m_fStepOverlap)	// overllap
	{
		for (blockStartX = window.x; blockStartX <= (window.x + window.width - cellWidth * blockWidth);
			blockStartX += cellWidth * m_fStepOverlap)	// overllap
		{
			cvGetCols(windowFeatureVector, &vector_block, startcol, startcol + 36);

			calcHOGForBlock(cvRect(blockStartX, blockStartY, cellWidth * blockWidth, cellHeight * blockHeight), &vector_block,
				integrals, cvSize(cellWidth, cellHeight), normalization);
...
CvMat* HOGProcessor::calcHOGFromImage(char* filename, CvSize window, int normalization)
{
	...
	imgFeatureVector = calcHOGWindow(integrals, cvRect(0, 0, window.width, window.height), normalization);
