...
CvPoint LineSegment::getIntersection(LineSegment *l1, LineSegment *l2)
{
	...
	// Kiem tra xem giao diem co nam trong 2 doan thang hay khong?
	if ((intersection.x >= l1->_begin.x && intersection.x <= l1->_end.x &&
		intersection.y >= l1->_begin.y && intersection.y <= l1->_end.y) &&
		(intersection.x >= l2->_begin.x && intersection.x <= l2->_end.x &&
			intersection.y >= l2->_begin.y && intersection.y <= l2->_end.y))
	{
		return intersection;
	}
